...
为保证后续阶段可持续推进，仓库默认采用以下分支与发布约定：

| 分支 | 用途 | 约束 |
| --- | --- | --- |
| `main` | 对齐上游主线 | 仅用于同步上游变更，不承载商业化改动 |
| `develop` | 日常集成开发 | 所有阶段性改动先进入此分支，保证可回放合并 |
| `main-custom` | 商业化与私有化发布 | 仅承载企业版、私有化和定制能力相关改动 |

发布与合并规则：

- 每个 Phase 完成后，在 `main-custom` 上创建对应阶段标签，便于回溯与回滚。
- 合并到 `develop` 前必须完成代码合并、文档更新、三端验证和回归检查。
- `main` 只跟随上游更新，不直接混入本仓库的定制逻辑。
- 若涉及跨阶段改动，优先在 `develop` 验证通过后，再同步到 `main-custom`。

建议的标签格式：`v{版本号}-{phase}`，例如 `v1.0.0-p0`、`v1.0.0-p1`。
